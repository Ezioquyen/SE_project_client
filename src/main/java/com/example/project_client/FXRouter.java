...
public final class FXRouter {
    ...
    public static void goTo(String routeLabel,VBox pane) throws IOException {
        RouteScene route = (RouteScene)routes.get(routeLabel);
        loadNewRoute(route,pane);
    }
    ...
    private static void loadNewRoute(RouteScene route, VBox pane) throws IOException {
        currentRoute = route;
        String scenePath = route.scenePath;
        Parent resource = FXMLLoader.load(FXRouter.class.getResource(scenePath));
        pane.getChildren().setAll(resource);
    }
